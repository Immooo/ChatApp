...
io.on('connection', (socket) => {
  ...
  // Emettre les utilisateurs actuels pour le canal spécifique quand un utilisateur rejoint
  ...
  socket.on('chat message', ({ message, isTyping }) => {
    ...
    const messageData = {
      id: Date.now(),
      username,
      timestamp: formattedTime, // ceci sera votre heure:min:sec
      msg: message,
      isTyping,
      edited: false,
    };
  ...
  socket.on('disconnect from channel', () => {
    ...
    // Emettre les utilisateurs actuels pour le canal spécifique quand un utilisateur quitte
